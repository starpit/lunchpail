...
func (model Model) report(c client) error {
	...
	for _, worker := range model.LiveWorkers {
		fmt.Fprintf(
			...
			len(worker.assignedTasks), len(worker.processingTasks), worker.nSuccess, worker.nFail, worker.name, c.PathArgs.RunName, worker.killfilePresent,
		)
	}
	for _, worker := range model.DeadWorkers {
		fmt.Fprintf(
			writer, "lunchpail.io\tdeadworker\t%d\t%d\t%d\t%d\t%s\t%s\n",
			len(worker.assignedTasks), len(worker.processingTasks), worker.nSuccess, worker.nFail, worker.name, c.PathArgs.RunName,
		)
	}
